...
export default function Navbar() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      // Fetch User Data
      const userResponse = await fetch(`/api/headerprofile`, {
        method: "GET",
        headers: {
      ...
      if (userResponse.ok) {
        ...
        const username = userData.user.name.length > 10
          ? userData.user.name.slice(0, 10) + "..."
          : userData.user.name;
        setUser({ ...userData.user, name: username });
      } else {
        console.error("Failed to fetch user data");
      }

      // Fetch Notifications Length
      const notificationsResponse = await fetch(`/api/notificationslength`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (notificationsResponse.ok) {
        ...
        setNotificationsLength(notificationsData.notificationslength || 0);
      } else {
        console.error("Failed to fetch notifications length");
      }
    };
        ...
        <Link href="/profile">
          <li className="cursor-pointer">
            <div className="flex flex-col items-center justify-center">
              {user?.profilePicUrl ? (
                <Image src={user.profilePicUrl} alt="Profile" width={30} height={30}  className="rounded-xl"/>
              ) : (
                <svg
                  xmlns="http://www.w3.org/2000/svg"
